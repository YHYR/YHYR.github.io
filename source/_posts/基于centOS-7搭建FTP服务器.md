...
修改FTP配置文件：`vi /etc/vsftpd/vsftpd.conf`，添加如下两行配置：
...
方案二：在设置userlist_deny为NO的同时，把需要授权的用户名添加到user_list中，并保证在ftpusers中不存在即可；例如上述例子中如果想让root用户成功登陆，则只需要把ftpusers中的root那一行删除即可。
...
在该目录下创建ftp_data/test和ftp_data/yhyr文件夹，分别代码test用户和yhyr用户所对应的数据存放目录，然后分别在test和yhyr目录下各创建一个文件，便于后面的演示，如下图所示：
...
顾名思义，该用户不是真实存在于系统中的用户；但是与匿名用户登录不同，虚拟用户登录时需要密码认证的。当你了解了上文提到过的本地用户登录的配置原理后，就会很容易理解虚拟用户的配置。
